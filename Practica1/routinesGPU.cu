...
__global__ void no_max_hough(uint8_t *,uint8_t *,float* , float* ,float , float , int , int ,float ,float , float , uint32_t *, int , int ,  float *, float *);
...
void line_asist_GPU(uint8_t *im, int height, int width, float *sin_table, float *cos_table, uint32_t *accum, int *x1, int *x2, int *y1, int *y2, int *nlines) {
	...
	cudaMemcpy(img_in_d, im, sizeIMG, cudaMemcpyHostToDevice);
	...
	//No max Supresion and Hough Transform
	...
	no_max_hough<<<dimGrid,dimBlock>>>(pedge_d, imgEdge_d, G_d, phi_d, lowthres, hithres, width, height, hough_h, center_x, center_y, accum_d, accu_width, accu_height, sin_table_d, cos_table_d);

	int threshold = height/6;
	if (width>height) threshold = width/6;
	cudaMemcpy(accum, accum_d, sizeACCUM, cudaMemcpyDeviceToHost);
...
__global__ void no_max_hough(uint8_t *pedge, uint8_t *img_out ,float* G, float* phi, float lowthres, float hithres, int width, int height, float hough_h, float center_x, float center_y, uint32_t *accumulators, int accu_width, int accu_height,  float *sin_table, float *cos_table) {
	...
	if(i < height && j < width) {
		if(img_out[ (i*width) + j] > 250) {						// Pixel is edge   
